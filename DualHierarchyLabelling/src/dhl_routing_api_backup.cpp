 ...
 *   DHL is a LABELING-BASED algorithm, not a search-based algorithm.
 *   It uses a two-phase approach for routing:
 * 
 *   Phase 1: DISTANCE COMPUTATION (Ultra-fast, O(log n))
 *     - Uses precomputed labels in ContractionIndex
...
vector<NodeID> ensure_snap_edges_in_path(
    ...
    if (!start_edge_included) {
        ...
        if (start_edge_oneway == 1) {
            // Forward: source→target, so we MUST traverse source→target
            // If we start at source, we need to prepend target AFTER source in path
            // If we start at target, we need to prepend source BEFORE it
            if (first_node == start_edge_source) {
                // We're at source, need to prepend target (to be second node)
                // This creates: source→target as first edge
                enhanced_path.push_back(start_edge_target);
            } else if (first_node == start_edge_target) {
                // We're at target, need to prepend source (to be first node)
                // This creates: source→target as first edge, then continues from target
                enhanced_path.push_back(start_edge_source);
            }
        } else if (start_edge_oneway == -1) {
            // Reverse: target→source, so we MUST traverse target→source
            if (first_node == start_edge_target) {
                // We're at target, need to prepend source (to be second node)
                enhanced_path.push_back(start_edge_source);
            } else if (first_node == start_edge_source) {
                // We're at source, need to prepend target (to be first node)
                enhanced_path.push_back(start_edge_target);
            }
        } else {
    ...
    if (!dest_edge_included) {
        ...
        if (dest_edge_oneway == 1) {
            ...
            if (last_node == dest_edge_source) {
                // We're at source, need to append target (to be last node)
                enhanced_path.push_back(dest_edge_target);
            } else if (last_node == dest_edge_target) {
                // We're at target, need to append source BEFORE it
                // This creates edge: source→target ending at target
                enhanced_path.push_back(dest_edge_source);
            }
        } else if (dest_edge_oneway == -1) {
            // Reverse: target→source, so we MUST arrive AT source
            if (last_node == dest_edge_target) {
                // We're at target, need to append source
                enhanced_path.push_back(dest_edge_source);
            } else if (last_node == dest_edge_source) {
                // We're at source, need to append target
                enhanced_path.push_back(dest_edge_target);
            }
        } else {
            // Bidirectional: allow either direction
...
void output_json_response(bool success, const string& error_message = "",
    ...
    if (!success) {
        ...
        for (size_t i = 0; i < edge_loop_end; i++) {
            ...
            // IMPORTANT: For same-edge cases, treat it only as first edge, not last
            bool is_last_edge = (i == path.size() - 2) && !is_same_edge_case;
            ...
            // CRITICAL: For first and last edges, ALWAYS use snap edges
            // Only use direct path for middle edges
            ...
            if (is_same_edge_case) {
                ...
                // For first edge, ALWAYS use start snap edge geometry
                auto snap_edge_forward = make_pair(start_edge_source, start_edge_target);
                auto snap_edge_backward = make_pair(start_edge_target, start_edge_source);
                ...
                // For last edge, ALWAYS use dest snap edge geometry
            ...
            } else if (edge_geometries.count(edge_key)) {
                // For middle edges, use direct path
            ...
            if (found_geometry && !coords_to_output.empty()) {
                ...
                // CRITICAL: We MUST output something for every edge, even if geometry is missing
                bool has_from = coordinates.count(from);
                bool has_to = coordinates.count(to);
                ...
                if (has_from || has_to) {
                    if (is_same_edge_case) {
                        ...
                        cout << "[" << fixed << setprecision(6) << start_snap_lng << ", " 
                             << start_snap_lat << "]";
                        if (has_to) {
                            cout << ", [" << coordinates.at(to).longitude << ", " 
                                 << coordinates.at(to).latitude << "]";
                        }
                    } else if (is_last_edge) {
                        // End at snap point
                        if (has_from) {
                            cout << "[" << fixed << setprecision(6) << coordinates.at(from).longitude << ", " 
                                 << coordinates.at(from).latitude << "], ";
                        }
                        cout << "[" << dest_snap_lng << ", " << dest_snap_lat << "]";
                    } else {
                        // Middle edge - use available node coordinates
                        if (has_from) {
                            cout << "[" << fixed << setprecision(6) << coordinates.at(from).longitude << ", " 
                                 << coordinates.at(from).latitude << "]";
                            if (has_to) cout << ", ";
                        ...
                        if (has_to) {
                            cout << "[" << fixed << setprecision(6) << coordinates.at(to).longitude << ", " 
                                 << coordinates.at(to).latitude << "]";
                        }
                    }
                } else {
                    // CRITICAL: No coordinates available - output minimal edge representation
                    // This ensures the edge is not completely lost
                    cout << "[" << fixed << setprecision(6) << 0.0 << ", " << 0.0 << "]";
                }
            }
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        cerr << "DEBUG: Original path from routing: ";
        for (auto n : path) cerr << n << " ";
        cerr << endl;
        
        cerr << "DEBUG: Start snap edge: " << start_edge_source << " -> " << start_edge_target 
             << " (oneway=" << start_edge_oneway << ")" << endl;
        cerr << "DEBUG: Dest snap edge: " << dest_edge_source << " -> " << dest_edge_target 
             << " (oneway=" << dest_edge_oneway << ")" << endl;
        ...
        // DEBUG: Print enhanced path
        cerr << "DEBUG: Enhanced path after snap edge fix: ";
        for (auto n : path) cerr << n << " ";
        cerr << endl;
        
        // DEBUG: Check if snap edges are actually in the path
        if (path.size() >= 2) {
            bool start_snap_in_path = false;
            for (size_t i = 0; i < path.size() - 1; i++) {
                if ((path[i] == start_edge_source && path[i+1] == start_edge_target) ||
                    (path[i] == start_edge_target && path[i+1] == start_edge_source)) {
                    start_snap_in_path = true;
                    break;
                }
            }
            cerr << "DEBUG: Start snap edge in path? " << (start_snap_in_path ? "YES" : "NO") << endl;
            
            bool dest_snap_in_path = false;
            for (size_t i = 0; i < path.size() - 1; i++) {
                if ((path[i] == dest_edge_source && path[i+1] == dest_edge_target) ||
                    (path[i] == dest_edge_target && path[i+1] == dest_edge_source)) {
                    dest_snap_in_path = true;
                    break;
                }
            }
            cerr << "DEBUG: Dest snap edge in path? " << (dest_snap_in_path ? "YES" : "NO") << endl;
        }
