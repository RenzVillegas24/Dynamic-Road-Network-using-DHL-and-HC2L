...
// Include base road network structures (DHL-specific extensions below)
...
// This file extends with DHL-specific structures and methods

//--------------------------- CutIndex (DHL-SPECIFIC) --------------------------
...
public:
    ...
    // remove isolated nodes from subgraph (inherited from BaseGraph)
    // reset graph to contain all nodes in global graph (inherited from BaseGraph)

    std::pair<distance_t, std::pair<NodeID, NodeID> > random_update();

    // Node/edge queries (inherited from BaseGraph):
    // node_count(), edge_count(), degree(), get_nodes(), get_edges()
