...
def merge_user_disruptions_with_traffic(flow_csv: Optional[str] = None, incident_csv: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if flow_csv or (not incident_csv):
            flow_path = Path(flow_csv) if flow_csv else get_latest_flow_csv()
        ...
        # Process incident CSV if available or requested
        if incident_csv or (not flow_csv):
            incident_path = Path(incident_csv) if incident_csv else get_latest_incident_csv()
