...
def fetch_here_traffic_data(api_key):
    """Fetch real-time traffic flow and incident data from HERE API."""
    if not api_key:
        print("⚠️  No HERE API key provided. Skipping real traffic data fetch.")
        return None
    
    bbox = "121.01,14.59,121.14,14.76"
    
    # Fetch Flow Data (congestion)
    flow_url = f"https://data.traffic.hereapi.com/v7/flow?in=bbox:{bbox}&locationReferencing=shape&apiKey={api_key}"
    flow_data = []
...
def process_here_disruptions(here_data, base_edges_df):
    """
    Convert HERE API data (flow + incidents) into disruption format for C++ routing APIs.
    
    Args:
        here_data: Dictionary with 'flow' and 'incidents' keys from HERE API
        base_edges_df: DataFrame with OSM edge data (source, target, length, etc.)
    
    Returns:
        DataFrame with columns: edge_id, disruption_type, severity, jam_factor, speed_reduction
    """
    if not here_data:
        print("⚠️  No HERE data to process")
        return pd.DataFrame()
    
    disruptions = []
    
    # Process Flow Data (Congestion)
    flow_segments = here_data.get('flow', [])
    print(f"\n🔧 Processing {len(flow_segments)} flow segments...")
    
    for flow in flow_segments:
        try:
            location = flow.get('location', {})
            segment_id = location.get('id', '')
            ...
            # Match to edge (simplified - in real implementation, use spatial matching)
            # For now, we'll create a record for each flow segment
            disruptions.append({
                'segment_id': segment_id,
                'disruption_type': 'congestion',
                'severity': 1 if jam_factor < 0.3 else 2 if jam_factor < 0.7 else 3,
    ...
    # Process Incident Data (Accidents, Closures)
    incidents = here_data.get('incidents', [])
    ...
    for incident in incidents:
        try:
            ...
            # Determine jam_factor and speed_reduction based on incident type
            if incident_type == 'ROAD_CLOSURE':
                jam_factor = 10.0  # Complete blockage (0-10 scale)
                speed_reduction = 1.0
                severity_level = 3
            elif incident_type == 'ACCIDENT':
                # Scale based on severity and traffic impact
            ...
            disruptions.append({
                ...
                'disruption_type': incident_type.lower(),
                'severity': severity_level,
                'jam_factor': jam_factor,
                'speed_reduction': speed_reduction,
                'source_speed': 0,  # Not applicable for incidents
                'free_flow_speed': 50  # Default assumption
            })
    ...
    print(f"✅ Processed {len(df)} total disruptions (flow + incidents)")
    
    return df


def generate_disruption_file_with_flow(base_edges_df, disruptions_df, output_file):
    """
    Generate enhanced disruption file with flow data for C++ routing APIs.
    Format: source target new_weight jam_factor current_speed free_flow_speed
    
    Args:
        base_edges_df: DataFrame with base graph edges
        disruptions_df: DataFrame with disruption data from HERE API
        output_file: Output .gr file path
    """
    print(f"\n📝 Generating flow-enhanced disruption file: {output_file}")
    ...
    # For each disruption, find matching edges and compute new weights
    disrupted_edges = []
    
    for _, disruption in disruptions_df.iterrows():
        # Simple matching: randomly select edges to apply disruptions
        # In production, use spatial matching with HERE location references
        sample_size = min(100, len(base_edges_df))
        sample_edges = base_edges_df.sample(n=sample_size, random_state=42)
        
        for _, edge in sample_edges.head(5).iterrows():  # Apply to 5 edges per disruption
    ...
    # Write to .gr file with flow data
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    
    with open(output_file, 'w') as f:
        f.write(f"c Flow-enhanced disruption file\n")
    ...
    print(f"✅ Wrote {len(disrupted_edges)} disrupted edges with flow data to {output_file}")
    print(f"   Format: source target new_weight jam_factor current_speed free_flow_speed")
