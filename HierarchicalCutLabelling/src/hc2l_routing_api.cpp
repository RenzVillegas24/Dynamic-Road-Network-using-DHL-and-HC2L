...
distance_t calculate_edge_cost(
    ...
    if (incident_data.count(edge_key)) {
        ...
        if (incident.road_closed) {
            return 999999999; // Infinite cost - road is impassable
        }
    }
    ...
    if (incident_data.count(edge_key)) {
        ...
        const TrafficFlowData* flow_ptr = flow_data.count(edge_key) ? &flow_data.at(edge_key) : nullptr;
        EdgeDisruptionMetrics metrics = compute_disruption_metrics(
            &incident,
            flow_ptr,
            highway_type,
            static_cast<distance_t>(cost)
        );
        
        // Apply weight multiplier from computed metrics
        cost *= metrics.weight_multiplier;
    }
...
void output_json_response(bool success, const string& error_message = "",
    ...
    if (!success) {
        ...
        for (size_t i = 0; i < edge_loop_end; i++) {
            ...
            // 8. Calculate edge duration based on traffic conditions
        ...
        for (size_t i = 0; i < path.size() - 1; i++) {
            ...
            if (g_disruption_cache.incidents.count(edge_key)) {
                ...
                } else if (metrics.severity_level == "high") {
                    route_disruptions_high++;
                } else if (metrics.severity_level == "medium") {
                    route_disruptions_medium++;
                } else if (metrics.severity_level == "low") {
        ...
        for (const auto& [edge, incident] : g_disruption_cache.incidents) {
            // Compute metrics for network-wide stats
            const TrafficFlowData* flow_ptr = nullptr;
            if (g_disruption_cache.flow_data.count(edge)) {
                flow_ptr = &g_disruption_cache.flow_data.at(edge);
            }
        ...
        // Sort by criticality and road closure (closures first, then by criticality)
        ...
        for (size_t i = 0; i < all_route_disruptions.size(); i++) {
            ...
            cout << "          \"time_impact_seconds\": " << fixed << setprecision(1) << metrics.time_impact_seconds << "," << endl;
            cout << "          \"old_weight\": " << metrics.old_weight << "," << endl;
            cout << "          \"new_weight\": " << metrics.new_weight << endl;
            cout << "        }" << endl;
            ...
            if (!load_success) {
                ...
                for (const auto& [edge_key, incident] : incident_data) {
                    ...
                    if (incident.road_closed || metrics.new_weight >= 999999.0) {
                        ...
                        double final_impact = metrics.impact_score;
                        ...
                        distance_t traffic_penalty = metrics.new_weight - base_weight;  // Additional cost from traffic
